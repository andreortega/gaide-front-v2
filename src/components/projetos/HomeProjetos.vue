  ...
  <v-container fluid>
    ...
    <v-row no-gutters class="pb-4 d-flex align-center">
      <v-col class="fill-height">
        <div class="text-h4 font-weight-regular text-tertiary">Projetos</div>
      </v-col>
      <v-col>
    ...
    <v-row class="">
      <v-col v-for="(p, i) in projetoStore.projetos" cols="12" sm="4" md="4">
        <v-hover>
          <template v-slot:default="{ isHovering, props }">
            ...
            >
              ...
              <v-row no-gutters class="pt-3 text-body-2 font-weight-mediumtext-grey-darken-2">
                <v-col>{{ p.empreendimentos_associados ? 'Tem Empreendimentos Associados' : 'Não Tem Empreendimentos Associados' }}</v-col>
              </v-row>
...
projetoStore.fill(route.params.id);

// AUTOCOMPLETE
//const search = ref('')
const values = ref(null);
const items = computed(() => {
  projetoStore.projetos;
});
